(function (global, factory) {
    ...
    var useQuery = (function (props) {
        var context = React.useContext(ApolloContext);
        var _a = React.useState(null), _ = _a[0], forceUpdate = _a[1];
        var query = React.useRef(new Query$1(props, context, forceUpdate));
        var client = getClient(props, context);
        React.useEffect(function () {
            query.current.componentDidMount(props);
            return query.current.componentWillUnmount;
        }, []);
        React.useEffect(query.current.removeQuerySubscription, [props.skip, props.query]);
        React.useEffect(function () {
    ...
    var invariant$a = require('invariant');
    var initialState$1 = {
        loading: false,
        called: false,
        error: undefined,
    ...
    var Mutation$1 = (function () {
        function Mutation(props, context) {
            ...
            this.mutate = function (options) {
                var _a = _this.props, mutation = _a.mutation, variables = _a.variables, optimisticResponse = _a.optimisticResponse, update = _a.update, _b = _a.context, context = _b === void 0 ? {} : _b, _c = _a.awaitRefetchQueries, awaitRefetchQueries = _c === void 0 ? false : _c;
                var mutateOptions = __assign$7({}, options);
                var refetchQueries = mutateOptions.refetchQueries || _this.props.refetchQueries;
                if (refetchQueries && refetchQueries.length && Array.isArray(refetchQueries)) {
                    refetchQueries = refetchQueries.map(function (x) {
                        if (typeof x === 'string' && _this.context.operations)
                ...
                var mutateVariables = Object.assign({}, variables, mutateOptions.variables);
                delete mutateOptions.variables;
                return _this.client.mutate(__assign$7({ mutation: mutation,
                    optimisticResponse: optimisticResponse,
                    refetchQueries: refetchQueries,
                    awaitRefetchQueries: awaitRefetchQueries,
                    update: update,
            ...
            this.verifyDocumentIsMutation = function (mutation) {
                var operation = parser(mutation);
                invariant$a(operation.type === DocumentType.Mutation, "The <Mutation /> component requires a graphql mutation, but got a " + (operation.type === DocumentType.Query ? 'query' : 'subscription') + ".");
            };
            this.props = props;
            this.context = context;
            this.client = getClient(props, context);
        ...
        Mutation.prototype.setState = function (state, cb) {
            this.state = __assign$7({}, this.state, state);
            if (cb) {
                cb();
            }
        };
    ...
    var useMutation = (function (props) {
        var context = React.useContext(ApolloContext);
        var mutation = React.useRef(new Mutation$1(props, context));
        React.useEffect(function () {
